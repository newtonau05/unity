...
public class CameraController : MonoBehaviour
{
    ...
    public GameObject cameraRotPanel;
    ...
    void Awake()
    {
        panelRect = (RectTransform)cameraRotPanel.GetComponent<RectTransform>();
    }
    ...
    void touchControl()
    {
        ...
        {
            if (rotateID == -1 && Input.GetTouch(i).phase == TouchPhase.Began)
            {
                if (RectTransformUtility.RectangleContainsScreenPoint(panelRect, Input.GetTouch(i).position, null))
                {
                    rotateID = i;
                }
            }
            else if (i <= rotateID && Input.GetTouch(i).phase == TouchPhase.Moved)
            {
                if (RectTransformUtility.RectangleContainsScreenPoint(panelRect, Input.GetTouch(i).position, null))
                {
                    Vector2 deltaPos = Input.GetTouch(i).deltaPosition;
                    float h = deltaPos.x;
                    float v = deltaPos.y;
                    rotateID = i;
            ...
            else if (i == rotateID && Input.GetTouch(i).phase == TouchPhase.Ended)
            {
                rotateID = -1;
            }
        }
